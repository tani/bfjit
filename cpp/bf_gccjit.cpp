...
void bfc(const std::string &bf_code) {
  ...
  std::array<gcc_jit_param*, 0> bf_func_params = {};
  auto bf_func = gcc_jit_context_new_function(ctx, nullptr, GCC_JIT_FUNCTION_EXPORTED, int_type, "bf", 0, bf_func_params.data(), 0);
  ...
  std::array<gcc_jit_param*, 3> memset_params = {
  ...
  auto memset_func = gcc_jit_context_new_function(ctx, nullptr, GCC_JIT_FUNCTION_IMPORTED, void_ptr_type, "memset", 3, memset_params.data(), 0);
  std::array<gcc_jit_rvalue*, 3> memset_args = {
  ...
  auto memset_func_call = gcc_jit_context_new_call(ctx, nullptr, memset_func, 3, memset_args.data());
  ...
  auto p_val = gcc_jit_context_new_cast(ctx, NULL, gcc_jit_lvalue_get_address(a_var, NULL), char_ptr_type);
    gcc_jit_block_add_assignment(block, NULL, p_var, p_val);
  std::array<gcc_jit_param*, 1> putchar_params = {
    gcc_jit_context_new_param(ctx, nullptr, int_type, "c")
  };
  auto putchar_func = gcc_jit_context_new_function(ctx, nullptr, GCC_JIT_FUNCTION_IMPORTED, int_type, "putchar", 1, putchar_params.data(), 0);
  std::array<gcc_jit_param*, 0> getchar_params = {};
  auto getchar_func = gcc_jit_context_new_function(ctx, nullptr, GCC_JIT_FUNCTION_IMPORTED, int_type, "getchar", 0, getchar_params.data(), 0);
  std::stack<std::pair<gcc_jit_block*, gcc_jit_block*>> loop_stack = {};
  for (char c : bf_code) {
    switch (c) {
      case '>': {
      ...
      case '[': {
        ...
        loop_stack.push(std::make_pair(cond_block, after_block));
        block = body_block;
        break;
      }
      case ']': {
        ...
        auto loop_blocks = loop_stack.top();
        loop_stack.pop();
